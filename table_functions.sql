...
returns table
as
    return (select r.registered_id, r.first_name, r.last_name, r.email_address, cda.is_student
...
returns table
as
    return (select * from dbo.conferences_data
...
returns table
as
    return (select * from conference_attendees_details(@conference_id) as cad
...
returns table
as
    return (select * from conference_day_attendees_details(@conference_day_id) as cad
           where cad.is_student = 0)
go
...
returns table
as
    return (select w.workshop_id, w.workshop_title, w.workshop_description, w.attendees_workshop_max,
...
returns table
as
    return (select c.conference_id, c.name, cd.conference_day_id, cd.date,
...
returns table
as
    return (select c.conference_id, c.name, cd.conference_day_id,
...
returns table
as
    return (select w.workshop_id, w.workshop_title, w.workshop_description
           from workshops w
           inner join workshop_reservations wr on w.workshop_id = wr.workshop_id
           inner join conference_day_reservations cdr on wr.reservation_day_id = cdr.reservation_day_id
           inner join reservations res on res.reservation_id = cdr.reservation_id
